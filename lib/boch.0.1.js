...
(function( window ){
  ...
  boch.prototype = {
    ...
    boching: function( name, callback ){
      ...
      try{
        ...
        setTimeout( function(){ allSetBoch( callback ); }, user_config[ 'interval_time' ] );  
  ...
  var getHtml = function( obj, callback ){
      ...
      if( xhr.readyState == 4 ){
        ...
        switch( xhr.status ){
          ...
          case 200 : 
            ...
            global_gethtml_initedcount++;
            setQueue( obj, xhr.responseText, callback ); 
          ...
          default :
            ...
            setQueue( obj, undefined, callback, function(){ alert( global_error_msg[ obj.name ] ); });
    ...
    function _timeout(){
      ...
      if( global_retry[ obj.name ] == user_config[ 'retry' ] ){
        ...
        setQueue( obj, undefined, callback );
      
        log( 'getHtml Error, name ' + obj.name + ' timeout retry count end ', global_retry[ obj.name ] );

      } else {
      
        setTimeout( function(){ getHtml.call( _this, obj, callback ) }, user_config[ 'interval_time' ] ); 
  ...
  }, setQueue = function( obj, html, callback ){
    ...
    function _boching(){ _boch.boching( obj.name, callback ); }
    ...
    util.each( global_queue, function( name ){
	  ...
	  function _boching(){ _boch.boching( name, callback ); }
      ...
      if( _is_setvalue.toString() === 'true' ){
        ...
        for( var _k in obj ){
          ...
          for( var i = 0, j = _data.length; i < j ; i++ ){
            ...
            _match_html[ _k ] = _before_div.innerHTML.match( new RegExp( "\\<data data-(" + _k + ")(.*?)\\>", "gim" ) );
          ...
          for( var _k in obj ){
            ...
            for( var i = 0, j = _data.length; i < j; i++ ){
              ...
              for( var _kk in _obj ){
                ...
                _match_html[ _kk ] = _before_div.innerHTML.match( new RegExp( "\\<data data-(" + _kk + ")(.*?)\\>", "gim" ) );
